...
class Pos extends MY_Controller
{
    ...
    function index($sid = null,$sale_order_id=null)
    {
        ...
        $this->form_validation->set_rules('reference_nob','reference_nob','is_unique[sales.reference_no]');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $total_discount = $this->erp->formatDecimal($order_discount + $total_discount);
				...
				if(isset($p_cur) && empty($_POST['amount'][0])){
					...
					for ($j = 0; $j < $p_cur; $j++) {
						$payment[] = array(
							...
							'pos_balance' => $this->erp->formatDecimal($p_amount - $paidd),
...
if($this->input->post('other_cur_paid') && $cur_rate->rate){
   $paid = $paid + ($this->input->post('other_cur_paid')/$cur_rate->rate);
				...
				if ($paid >= $grand_total) {
					$payment_status = 'paid';
				} elseif ($grand_total > $paid && $paid > 0) {
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if ($suspend) {
				...
				*/
				...
				//$this->erp->print_arrays($payment_status);
