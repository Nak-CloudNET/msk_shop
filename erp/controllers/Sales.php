...
class Sales extends MY_Controller
{
	...
	function customer_balance_actions__old()
	{
		if (!$this->Owner) {
			$this->session->set_flashdata('warning', lang('access_denied'));
			redirect($_SERVER["HTTP_REFERER"]);
	...
	function p_invoice_old($id = NULL)
    {
		$this->erp->checkPermissions('index');
    ...
    function p_invoice_logo($id = NULL,$cash,$check)
    {
        $this->erp->checkPermissions('index');
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        ...
        $this->data['cash'] = $cash;
        $this->data['check'] = $check;
        $this->load->view($this->theme . 'sales/p_invoice_logo', $this->data);
    }

    function p_receipt_logo($id = NULL,$cash,$check)
    {
        $this->erp->checkPermissions('index');
        ...
        $this->data['check'] = $check;
        $this->data['cash'] = $cash;
        $this->load->view($this->theme . 'sales/p_receipt_logo', $this->data);
    }


    function p_invoice_receipt($id = NULL,$cash,$check,$title)
    {
        $this->erp->checkPermissions('index');
        ...
        $this->load->view($this->theme . 'sales/p_invoice_receipt', $this->data);
    }
	
	function tax_invoice_receipt($id = NULL)
    {
		$this->erp->checkPermissions('index');
        ...
        $this->data['rows'] = $this->sales_model->getAllInvoiceItems($id);
		$this->data['logo'] = true;
        $this->load->view($this->theme . 'sales/tax_invoice_receipt', $this->data);
    }
    ...
    function return_sale($id = NULL)
    {
        ...
        $this->form_validation->set_rules('reference_no', lang("reference_no"), 'trim|required|is_unique[return_sales.reference_no]');
   ...
   public function combine_pdf($sales_id)
	...
	function combine_payment($status)
    {
    ...
    function getDeliveryAlerts($warehouse_id = NULL){
        ...
        $this->load->library('datatables');
            ...
            ->where('CURDATE() >= DATE_SUB(delivery_date , INTERVAL (SELECT alert_day FROM erp_settings) DAY)')
	...
	function add_new_delivery() {
        ...
        $this->form_validation->set_rules('delivery_by', lang("delivery_by"), 'trim|required|is_natural_no_zero');
        ...
        // $this->erp->print_arrays($delivery);exit;
            ...
            $this->session->set_flashdata('error', lang("Delivery Reference No field is required / must contain a unique value."));
            redirect($_SERVER["HTTP_REFERER"]);
        }
...
//        $this->data['csrf'] = $this->_get_csrf_nonce();
	...
	function modal_view_pos($id = NULL)
    {
        ...
        $this->load->view($this->theme.'sales/modal_view_pos', $this->data);
    }
