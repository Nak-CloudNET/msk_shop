	...
	public function getCategoryValueByID($id) {
        $this->db
                ->select('
                        ...
                        COALESCE(SUM(price*quantity) - SUM(cost*quantity), 0) as balance')
                    ->from('categories')
	...
	*/
	...
	public function getsale_top_export($id){
		$this->db->select("erp_sale_items.id, 
			...
			SUM(COALESCE(erp_sale_items.quantity,0) * COALESCE(erp_sale_items.unit_price,0)) as amount,			
			(erp_units.name)")
			->from('erp_sale_items') 
	...
	public function getProductsProfitByProduct($warehouse_id,$category_id,$product_id,$from_date,$to_date,$reference_no, $biller){
		$this->db->select('erp_sales.date,erp_sales.customer,erp_sales.reference_no,erp_sale_items.*, companies.name AS biller_name');
