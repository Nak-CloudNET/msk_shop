...
class Site extends CI_Model
{
	...
	*/
	...
	public function get_delivery_alerts(){
		$this->db->select('COUNT(*) AS count');
		$this->db->where('CURDATE() >= DATE_SUB(delivery_date , INTERVAL (SELECT alert_day FROM erp_settings) DAY)');
...
/*	
 saram	public function getSuspendByID($id){
	...
	*/
            ...
            switch ($field) {
                ...
                case 'pos':
					if($biller_prefix){
						...
						$prefix = isset($this->Settings->sales_prefix) && $this->Settings->sales_prefix != null ? $this->Settings->sales_prefix . '/POS' : 'POS';
						break;
					}
                case 're':
                    if($biller_prefix){
	...
	public function getReference_old($field, $biller = null) {
            ...
            switch ($field) {
                ...
                case 'pos':	
					if($biller_prefix){
						...
						$prefix = isset($this->Settings->sales_prefix) ? 'P' : '';
						break;
					}
                case 're':
                    if($biller_prefix){
	...
	*/
            ...
            foreach ($warehouses as $warehouse) {
                ...
                if($type != 'service'){
					...
					if($this->getWarehouseProduct($product_id, $warehouse->id)){
						$this->db->update('warehouses_products', array('quantity' => 1), array('warehouse_id'=>$warehouse->id, 'product_id' => $product_id));
					}else{
						$this->db->insert('warehouses_products', array('warehouse_id'=>$warehouse->id, 'product_id' => $product_id, 'quantity' => 1));
					}
				}
	...
	public function setSyncInvPaidTrigger($id) {
		$sale = $this->getSaleByID($id);
		$updated_count = $sale->updated_count + 1;
		$this->db->update('sales', array('inv_paid_edit' => '1', 'updated_count' => $updated_count), array('id' => $id));
		$this->db->update('sale_items', array('inv_paid_edit' => '1', 'updated_count' => $updated_count), array('sale_id' => $id));
	}
