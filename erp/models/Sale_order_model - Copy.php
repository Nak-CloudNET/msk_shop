...
class Sale_order_model extends CI_Model
{
	...
	public function getInvoiceByID($id)
    {
		...
		$this->db->select("sale_order.*, companies.name AS customer, users.username AS saleman,delivery.name as delivery_man,grand_total, paid,(grand_total-paid) as balance")
				 ->join('companies', 'companies.id = sale_order.customer_id', 'left')
				 ->join('users', 'users.id = sale_order.saleman_by', 'left')
				 ->join('companies as delivery', 'delivery.id = sale_order.delivery_by', 'left')
				 ->join('deliveries', 'deliveries.sale_id = sale_order.id', 'left')
    ...
    {
        $this->db->select('erp_deliveries.*, erp_delivery_items.product_name as description, delivery_items.quantity_received as qty, erp_companies.name, erp_units.name as unit, delivery_items.category_name as brand, delivery_items.option_id, product_variants.name as variant, (erp_delivery_items.quantity_received * erp_product_variants.qty_unit) as variant_qty');
