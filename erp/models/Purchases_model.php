...
class Purchases_model extends CI_Model
{
	...
	public function getPurTranNo($id){
		...
		$this->db->join('gl_trans','erp_gl_trans.reference_no = erp_purchases.reference_no AND erp_purchases.biller_id = erp_gl_trans.biller_id','inner');
		$this->db->where('purchases.id', $id);
		$this->db->where('gl_trans.tran_type','PURCHASE EXPENSE');
		$this->db->or_where('gl_trans.tran_type','PURCHASE');
		$q = $this->db->get();
		if($q->num_rows() > 0){
			return  $q->row();
		}
	...
	public function getPurchaseJournalByTranNo($tran_no){
		$this->db->select('gl_trans.*, (IF(erp_gl_trans.amount > 0, erp_gl_trans.amount, null)) as debit, 
							(IF(erp_gl_trans.amount < 0, abs(erp_gl_trans.amount), null)) as credit');
		$this->db->where("tran_no",$tran_no);
		$this->db->where("tran_type","PURCHASE EXPENSE");
		$this->db->or_where("tran_type","PURCHASE");
	...
	public function updateJournal($reference = NULL, $rows = array()) {
		...
		$this->db->delete('gl_trans', array('tran_type' => 'PURCHASE EXPENSE', 'reference_no' => $reference));
